...
Make it authentic, engaging, and optimized for Instagram algorithm."""
        ...
        try:
            ...
            if "404" in error_msg or "NOT_FOUND" in error_msg:
                logger.error("=" * 60)
                logger.error(f"❌ MODEL NOT FOUND: '{self.model_name}' is not available!")
                logger.error("=" * 60)
                logger.error("\n🔍 The Gemini model in your config.yaml is not found.")
                logger.error("   This happens when:")
                logger.error("   • Model name is outdated or incorrect")
                logger.error("   • Model is not available for your API key/region")
                logger.error("   • Model requires different API version or permissions")
                logger.error("\n💡 SOLUTION:")
                logger.error("   1. Run the model list helper to see what's available:")
                logger.error("      python list_gemini_models.py")
                logger.error("\n   2. Update config.yaml with an available model:")
                logger.error("      api:")
                logger.error("        gemini_model: \"gemini-1.5-flash\"  # Example")
                logger.error("\n   3. Common working models to try:")
                logger.error("      • gemini-1.5-flash (recommended, fast)")
                logger.error("      • gemini-1.5-pro (more capable)")
                logger.error("      • gemini-pro (stable, older)")
                logger.error("      • gemini-1.0-pro (legacy)")
                logger.error("\n   4. For GitHub Actions:")
                logger.error("      Update the gemini_model in config.yaml and push changes")
                ...
                logger.error("   Get a new key from: https://aistudio.google.com/app/apikey")
                ...
                logger.error("   Check quota at: https://aistudio.google.com/")
